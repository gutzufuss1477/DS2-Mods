...
$name = 'DS2_Weapons_Anywhere_v1.0.0'
...
Copy-Item -LiteralPath (Join-Path $root 'build\public\ds2_weapons_anywhere.asi') -Destination $stage -Force
Copy-Item -LiteralPath (Join-Path $root 'build\public\ds2_weapons_anywhere.ini') -Destination $stage -Force
...
Copy-Item -LiteralPath (Join-Path $root 'docs\TEST_PLAN.md') -Destination (Join-Path $stageDocs 'TEST_PLAN.md') -Force
Copy-Item -LiteralPath (Join-Path $root 'docs\VALIDATION.md') -Destination (Join-Path $stageDocs 'VALIDATION.md') -Force
Copy-Item -LiteralPath (Join-Path $root 'docs\TECHNICAL_NOTES.md') -Destination (Join-Path $stageDocs 'TECHNICAL_NOTES.md') -Force

$asiHash = (Get-FileHash -LiteralPath (Join-Path $stage 'ds2_weapons_anywhere.asi') -Algorithm SHA256).Hash.ToUpperInvariant()
@(
    "$asiHash  ds2_weapons_anywhere.asi"
    "$((Get-FileHash -LiteralPath (Join-Path $stage 'ds2_weapons_anywhere.ini') -Algorithm SHA256).Hash.ToUpperInvariant())  ds2_weapons_anywhere.ini"
