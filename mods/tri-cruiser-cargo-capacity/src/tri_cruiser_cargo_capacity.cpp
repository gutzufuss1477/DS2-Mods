...
static BOOL patchJump(u8* at,u8* stub){
    if(!rel32ok((u64)at+5,(u64)stub)) return FALSE;
    u8 p[5]={0xE9,0,0,0,0};
    put32(p+1,rel32((u64)at+5,(u64)stub));
    return writeMem(at,p,5);
}

struct StubDef{const u8* start;const u8* end;const u8* ret;u64 hookRva;};
static StubDef stubs[9]={
    {SKeyAStubStart,SKeyAStubEnd,SKeyAReturnPtr,RVA_HOOK[0]},
    {SKeyBStubStart,SKeyBStubEnd,SKeyBReturnPtr,RVA_HOOK[1]},
    {MKeyAStubStart,MKeyAStubEnd,MKeyAReturnPtr,RVA_HOOK[2]},
    {MKeyBStubStart,MKeyBStubEnd,MKeyBReturnPtr,RVA_HOOK[3]},
    {LKeyAStubStart,LKeyAStubEnd,LKeyAReturnPtr,RVA_HOOK[4]},
    {LKeyBStubStart,LKeyBStubEnd,LKeyBReturnPtr,RVA_HOOK[5]},
    {XLKeyAStubStart,XLKeyAStubEnd,XLKeyAReturnPtr,RVA_HOOK[6]},
    {XLKeyBStubStart,XLKeyBStubEnd,XLKeyBReturnPtr,RVA_HOOK[7]},
    {ItemCapStubStart,ItemCapStubEnd,ItemCapReturnPtr,RVA_ITEMCAP}
};
static const u32 STUB_STRIDE=0x100;
static const SIZE_T CAVE_SIZE=0x1000;
...
static u8* allocNear(u64 target){
    ...
    for(u64 d=step;d<maxd;d+=step){
        ...
        for(u32 k=0;k<2;k++){
            ...
            if(!p) continue;
            BOOL ok=TRUE;
            for(u32 i=0;i<9;i++) if(!rel32ok(g_base+stubs[i].hookRva+5,(u64)p+i*STUB_STRIDE)){ok=FALSE;break;}
            if(ok) return p;
...
static BOOL install(void){
    ...
    // Fail closed on an unsupported executable or conflicting patch.
    ...
    for(u32 i=0;i<9;i++){
        ...
        put64(cave+i*STUB_STRIDE+ro,g_base+stubs[i].hookRva+5);
    }
    if(!FlushInstructionCache(GetCurrentProcess(),cave,CAVE_SIZE)){VirtualFree(cave,0,MEM_RELEASE);g_cave=0;return FALSE;}

    // 160 units per native side, using same-length register compares.
    const u8 P_A0[6]={0x41,0xB8,0xA0,0x00,0x00,0x00};
    const u8 P_A1[3]={0x44,0x39,0xC1};
    const u8 P_B0[6]={0x41,0xB8,0xA0,0x00,0x00,0x00};
    const u8 P_B1[3]={0x44,0x39,0xC7};

    if(!writeMem((u8*)(g_base+RVA_PACK_A0),P_A0,6)) goto fail; g_patchMask|=1;
    if(!writeMem((u8*)(g_base+RVA_PACK_A1),P_A1,3)) goto fail; g_patchMask|=2;
    if(!writeMem((u8*)(g_base+RVA_PACK_B0),P_B0,6)) goto fail; g_patchMask|=4;
    if(!writeMem((u8*)(g_base+RVA_PACK_B1),P_B1,3)) goto fail; g_patchMask|=8;
    for(u32 i=0;i<8;i++){
        if(!patchJump((u8*)(g_base+RVA_HOOK[i]),cave+i*STUB_STRIDE)) goto fail;
        g_patchMask|=(1L<<(i+4));
    }
    if(!patchJump((u8*)(g_base+RVA_ITEMCAP),cave+8*STUB_STRIDE)) goto fail;
    g_patchMask|=(1L<<12);
    return TRUE;
