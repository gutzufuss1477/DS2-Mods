...
2. Die bisherige High-Density-Backpack-ASI aus dem aktiven Mod-Ordner nehmen und die einzelne ASI aus dem test2-ZIP einsetzen. Vorhandenen ASI-Loader weiterverwenden.
3. Zuerst den zuvor abst�rzenden Spielstand laden, einige Minuten spielen und das Rucksackmen� �ffnen. Im Log muss `v1.1.0-test2` stehen.
4. Im Log `DS2_HighDensityBackpackModules.log` nach `status=PATCH_APPLIED` schauen. Bei `...REJECTED...` bitte das Log aufbewahren.
