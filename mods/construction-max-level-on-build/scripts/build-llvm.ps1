...
$source = Join-Path $root 'src\construction_max_level_on_build.cpp'
$definitions = Join-Path $root 'src\kernel32.def'
$configuration = Join-Path $root 'config\ds2_construction_max_level_on_build.ini'
...
$object = Join-Path $out 'construction_max_level_on_build.obj'
...
$asi = Join-Path $out 'ds2_construction_max_level_on_build.asi'
...
Copy-Item -LiteralPath $configuration -Destination (Join-Path $out 'ds2_construction_max_level_on_build.ini') -Force
