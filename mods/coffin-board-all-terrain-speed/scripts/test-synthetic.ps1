...
public static class CoffinNativeTest {
...
$module = [CoffinNativeTest]::LoadLibraryW($dll)
if ($module -eq [IntPtr]::Zero) { throw "LoadLibrary failed: $([Runtime.InteropServices.Marshal]::GetLastWin32Error())" }
try {
    $address = [CoffinNativeTest]::GetProcAddress($module, 'RunSyntheticCoffinSpeedTest')
    if ($address -eq [IntPtr]::Zero) { throw 'test export missing' }
    $test = [Runtime.InteropServices.Marshal]::GetDelegateForFunctionPointer($address, [type][CoffinNativeTest+RunTest])
    $result = $test.Invoke()
    if ($result -ne 0) { throw "synthetic Coffin Board test failed: $result" }
    Write-Host 'Synthetic Coffin Board runtime test: PASS'
} finally {
    [void][CoffinNativeTest]::FreeLibrary($module)
}
