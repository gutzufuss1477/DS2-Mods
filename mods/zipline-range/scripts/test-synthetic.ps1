...
Copy-Item -LiteralPath (Join-Path $root 'config\ds2_zipline_range.ini') -Destination (Join-Path $out 'ds2_zipline_range.ini') -Force

Add-Type -TypeDefinition @'
using System;
using System.Runtime.InteropServices;

public static class ZiplineNativeTest {
...
$module = [ZiplineNativeTest]::LoadLibraryW($dll)
if ($module -eq [IntPtr]::Zero) { throw "LoadLibrary failed: $([Runtime.InteropServices.Marshal]::GetLastWin32Error())" }
$image = [IntPtr]::Zero
try {
    $settingsProc = [ZiplineNativeTest]::GetProcAddress($module, 'RunPackagedSettingsTest')
    $syntheticProc = [ZiplineNativeTest]::GetProcAddress($module, 'RunSyntheticZiplineTest')
    if ($settingsProc -eq [IntPtr]::Zero -or $syntheticProc -eq [IntPtr]::Zero) { throw 'test export missing' }
    $settingsTest = [Runtime.InteropServices.Marshal]::GetDelegateForFunctionPointer(
        $settingsProc, [type][ZiplineNativeTest+NoArgs]
    )
    $syntheticTest = [Runtime.InteropServices.Marshal]::GetDelegateForFunctionPointer(
        $syntheticProc, [type][ZiplineNativeTest+WithImage]
    )
    ...
    $image = [ZiplineNativeTest]::VirtualAlloc([IntPtr]::Zero, [UIntPtr]$capacity, 0x3000, 0x04)
    ...
    if ($image -ne [IntPtr]::Zero) { [void][ZiplineNativeTest]::VirtualFree($image, [UIntPtr]::Zero, 0x8000) }
    [void][ZiplineNativeTest]::FreeLibrary($module)
}
