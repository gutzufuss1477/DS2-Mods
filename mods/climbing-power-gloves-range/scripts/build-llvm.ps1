...
$source = Join-Path $root 'src\climbing_gloves_range.cpp'
$config = Join-Path $root 'config\ds2_climbing_gloves_range.ini'
foreach ($input in @($source, $config)) {
    if (!(Test-Path -LiteralPath $input)) { throw "Required build input missing: $input" }
}
...
$object = Join-Path $out 'climbing_gloves_range.obj'
...
$asi = Join-Path $out 'ds2_climbing_gloves_range.asi'
...
Copy-Item -LiteralPath $config -Destination (Join-Path $out 'ds2_climbing_gloves_range.ini') -Force
