...
typedef unsigned char u8; typedef unsigned int u32; typedef unsigned long long u64; typedef wchar_t WCHAR;
...
HANDLE WINAPI CreateMutexW(LPVOID,BOOL,LPCWSTR);
HMODULE WINAPI GetModuleHandleW(LPCWSTR); void* WINAPI GetProcAddress(HMODULE,LPCSTR); DWORD WINAPI GetModuleFileNameW(HMODULE,LPWSTR,DWORD); HANDLE WINAPI GetCurrentProcess(void); DWORD WINAPI GetCurrentProcessId(void); DWORD WINAPI GetCurrentThreadId(void); HANDLE WINAPI CreateThread(LPVOID,SIZE_T,LPTHREAD_START_ROUTINE,LPVOID,DWORD,DWORD*); BOOL WINAPI CloseHandle(HANDLE); void WINAPI Sleep(DWORD); SIZE_T WINAPI VirtualQuery(LPCVOID,MEMORY_BASIC_INFORMATION*,SIZE_T); LPVOID WINAPI VirtualAlloc(LPVOID,SIZE_T,DWORD,DWORD); BOOL WINAPI VirtualFree(LPVOID,SIZE_T,DWORD); BOOL WINAPI VirtualProtect(LPVOID,SIZE_T,DWORD,DWORD*); BOOL WINAPI FlushInstructionCache(HANDLE,LPCVOID,SIZE_T); BOOL WINAPI ReadProcessMemory(HANDLE,LPCVOID,LPVOID,SIZE_T,SIZE_T*); HANDLE WINAPI CreateFileW(LPCWSTR,DWORD,DWORD,LPVOID,DWORD,DWORD,HANDLE); BOOL WINAPI ReadFile(HANDLE,LPVOID,DWORD,DWORD*,LPVOID); BOOL WINAPI WriteFile(HANDLE,LPCVOID,DWORD,DWORD*,LPVOID); BOOL WINAPI FlushFileBuffers(HANDLE); BOOL WINAPI GetFileSizeEx(HANDLE,LARGE_INTEGER*); DWORD WINAPI SetFilePointer(HANDLE,LONG,LONG*,DWORD); BOOL WINAPI DisableThreadLibraryCalls(HMODULE); BOOL WINAPI QueryPerformanceCounter(LARGE_INTEGER*); BOOL WINAPI QueryPerformanceFrequency(LARGE_INTEGER*); void WINAPI GetLocalTime(SYSTEMTIME*); HANDLE WINAPI CreateToolhelp32Snapshot(DWORD,DWORD); BOOL WINAPI Thread32First(HANDLE,THREADENTRY32*); BOOL WINAPI Thread32Next(HANDLE,THREADENTRY32*); HANDLE WINAPI OpenThread(DWORD,BOOL,DWORD); DWORD WINAPI SuspendThread(HANDLE); DWORD WINAPI ResumeThread(HANDLE); BOOL WINAPI GetThreadContext(HANDLE,CONTEXT*); BOOL WINAPI SetThreadContext(HANDLE,const CONTEXT*); void WINAPI GetSystemInfo(SYSTEM_INFO*); HANDLE WINAPI HeapCreate(DWORD,SIZE_T,SIZE_T); LPVOID WINAPI HeapAlloc(HANDLE,DWORD,SIZE_T); LPVOID WINAPI HeapReAlloc(HANDLE,DWORD,LPVOID,SIZE_T); BOOL WINAPI HeapFree(HANDLE,DWORD,LPVOID); BOOL WINAPI HeapDestroy(HANDLE); DWORD WINAPI GetLastError(void); DWORD WINAPI GetPrivateProfileStringW(LPCWSTR,LPCWSTR,LPCWSTR,LPWSTR,DWORD,LPCWSTR); void WINAPI ExitProcess(UINT);
#ifdef __cplusplus
}
#endif
