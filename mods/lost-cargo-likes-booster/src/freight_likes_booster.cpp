...
static LPVOID g_veh;
static volatile LONG g_active,g_shutdown,g_stepTid,g_bonusArmed;
static volatile u64 g_extraLikes=5000,g_connTarget=0,g_pendingRecord=0;
...
static void clearPending(){g_pendingRecord=0;g_pendingTid=0;InterlockedExchange(&g_bonusArmed,0);}
...
static LONG WINAPI veh(EXCEPTION_POINTERS*e){
    ...
    if((dr6&1)&&rip==like){
        ...
        if(eligible&&InterlockedCompareExchange(&g_bonusArmed,1,0)==0){g_pendingRecord=r9;g_pendingTid=tid;cs(c,RCXOFF,in+g_extraLikes);}
    ...
    if(conn&&(dr6&2)&&rip==conn){
        ...
        if(g_bonusArmed&&record==g_pendingRecord&&tid==g_pendingTid&&cOk&&vOk){u64 expected=(u64)current+g_extraLikes;if(expected<=0xFFFFFFFFULL&&visible==expected){wr32(rsi+8,(u32)visible);}clearPending();}
        g_stepTid=(LONG)tid;cs(c,EFLAGSOFF,cg(c,EFLAGSOFF)|0x100ULL);return EXCEPTION_CONTINUE_EXECUTION;
    }
