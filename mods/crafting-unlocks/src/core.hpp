...
struct Settings { bool enabled; bool all; bool dump; bool freeCrafting; bool durabilityEnabled; bool durabilityUnbreakable; u32 durabilityMilli; u32 count; Rule rules[MaxRules]; };
...
inline bool milli(const char*p,u32 n,u32&out){trim(p,n);if(!n)return false;u32 whole=0,i=0,digits=0;while(i<n&&p[i]>='0'&&p[i]<='9'){if(whole>1000)return false;whole=whole*10+u32(p[i++]-'0');++digits;}if(!digits)return false;u32 frac=0,fd=0;if(i<n&&p[i]=='.'){++i;while(i<n&&p[i]>='0'&&p[i]<='9'&&fd<3){frac=frac*10+u32(p[i++]-'0');++fd;}if(i<n&&p[i]>='0'&&p[i]<='9')return false;}if(i!=n)return false;while(fd<3){frac*=10;++fd;}if(whole>1000||(whole==1000&&frac))return false;out=whole*1000+frac;return out>=1000&&out<=1000000;}
inline u32 milliFloatBits(u32 m){u32 e=0;while(e<9&&((u64)1000<<(e+1))<=m)++e;u64 den=(u64)1000<<e,num=(u64)m*0x800000ull,q=num/den,rem=num%den;if(rem*2>den||(rem*2==den&&(q&1)))++q;if(q>=0x1000000ull){++e;q=0x800000ull;}return ((e+127u)<<23)|u32(q-0x800000ull);}

inline bool parse(const char* p,u32 n,Settings&s,Error&e){
 s.enabled=false;s.all=false;s.dump=true;s.freeCrafting=false;s.durabilityEnabled=false;s.durabilityUnbreakable=false;s.durabilityMilli=2000;s.count=0;e={0,nullptr};
 u32 pos=0,line=0,section=0,seen=0,seenDur=0;if(n>=3&&(u8)p[0]==239&&(u8)p[1]==187&&(u8)p[2]==191)pos=3;
 while(pos<n){++line;u32 begin=pos;while(pos<n&&p[pos]!='\n'){if(p[pos]==0){e={line,"NUL/UTF-16 is not supported; use UTF-8"};return false;}++pos;}u32 len=pos-begin;if(pos<n)++pos;const char*t=p+begin;
  ...
  if(t[0]=='['){if(len<3||t[len-1]!=']'){e={line,"invalid section"};return false;}const char*a=t+1;u32 l=len-2;trim(a,l);if(eq(a,l,"CraftingUnlocks"))section=1;else if(eq(a,l,"Items"))section=2;else if(eq(a,l,"Durability"))section=3;else{e={line,"unknown section"};return false;}continue;}
  ...
  if(section==1){if(value<0){e={line,"global values must be 0 or 1"};return false;}u32 bit=eq(k,kl,"Enabled")?1:eq(k,kl,"DefaultUnlock")?2:eq(k,kl,"ExportCatalogue")?4:eq(k,kl,"FreeCrafting")?8:0;if(!bit||(seen&bit)){e={line,"unknown or duplicate global setting"};return false;}seen|=bit;if(bit==1)s.enabled=value!=0;else if(bit==2)s.all=value!=0;else if(bit==4)s.dump=value!=0;else s.freeCrafting=value!=0;}
  else if(section==3){u32 bit=eq(k,kl,"Enabled")?1:eq(k,kl,"Multiplier")?2:eq(k,kl,"Unbreakable")?4:0;if(!bit||(seenDur&bit)){e={line,"unknown or duplicate durability setting"};return false;}seenDur|=bit;if(bit==2){u32 mv=0;if(!milli(v,vl,mv)){e={line,"Multiplier must be 1.0 to 1000.0"};return false;}s.durabilityMilli=mv;}else{if(value<0){e={line,"durability toggles must be 0 or 1"};return false;}if(bit==1)s.durabilityEnabled=value!=0;else s.durabilityUnbreakable=value!=0;}}
...
inline Skip eligibility(const Settings&s,const Recipe&r,u8 mask){
 ...
 if(!supported_usage(r.usage)&&!(r.usage==0&&special_normal_recipe(r.key)))return Skip::UnsupportedPath;
 ...
 if(!special_normal_recipe(r.key)&&!facility_allows(r.usage,mask))return Skip::Facility;
 return Skip::None;
}
