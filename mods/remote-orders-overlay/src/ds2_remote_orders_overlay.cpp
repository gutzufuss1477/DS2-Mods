// DS2 Remote Orders v0.1.2-alpha-overlay-fix
...
static u8 g_nativeDisplayScratch[MAX_NATIVE_DISPLAY_SCRATCH];
...
static bool ResolveNativeFacilityName(u64 resource,DisplayRow*row){
    ...
    __try{format(&formatted,0,g_imageBase+RVA_FACILITY_FORMAT_DATA,hash);WCHAR target[MAX_TEXT_CHARS];MemSet(target,0,sizeof(target));if(CopyNativeUtf8ToWide(formatted,target,MAX_TEXT_CHARS)){CopyW(row->title,MAX_TEXT_CHARS,target);usize n=StrLenW(row->title);if(n>1&&(n%2u)==0){usize h=n/2u;bool same=true;for(usize i=0;i<h;++i)if(row->title[i]!=row->title[h+i]){same=false;break;}if(same)row->title[h]=0;}ok=true;}}__except(1){AtomicInc(&g_nativeCallFaults);}
...
static bool ResolveNativeMissionNames(u64 mission,DisplayRow*row){
    ...
    usize titleLen=StrLenW(row->title);
    if(titleLen>1&&(titleLen%2u)==0){usize half=titleLen/2u;bool same=true;for(usize i=0;i<half;++i)if(row->title[i]!=row->title[half+i]){same=false;break;}if(same)row->title[half]=0;}
    row->subtitle[0]=0;row->kind[0]=0;return true;
}

static void MakeMissionRowLabel(DisplayRow*row){
    if(!row)return;
    if(!row->title[0]){CopyW(row->title,MAX_TEXT_CHARS,L"Auftrag");}
    row->subtitle[0]=0;
}
...
static void CopyScanToSnapshot(DisplaySnapshot*out){
    ...
    if(g_scan.complete){if(out->totalAvailable==0)CopyW(out->status,192,L"Keine aktuell verfügbaren Aufträge");else CopyW(out->status,192,L"Sicherer Textmodus: IDs und Auftragstypen");}
    else{CopyW(out->status,192,L"Aufträge werden geprüft: ");AppendDecW(out->status,192,g_scan.nextIndex);AppendCharW(out->status,192,L'/');AppendDecW(out->status,192,g_scan.sourceCount);}
}
...
static void BuildDisplaySnapshot(DisplaySnapshot*out){
    ...
    if(!ResolveSelectedFacility(&sel)){g_uiPendingSelection=0;g_uiPendingSince=0;g_uiLastBuiltSelection=0;MemSet(&g_scan,0,sizeof(g_scan));CopyW(out->status,192,L"Weltkarte öffnen und über eine Einrichtung hovern");return;}
...
static void DrawOverlay(HWND hwnd){AtomicInc(&g_overlayPaintCount);
    ...
    int right=(int)client.right-12;if(right<260)right=260;
    pSetTextColor(dc,RGBc(238,241,244));HGDIOBJ old=pSelectObject(dc,g_headingFont);
    WCHAR headingLine[160];CopyW(headingLine,160,L"Remote-Aufträge  ·  Einrichtung: ");AppendSignedW(headingLine,160,snap.facilityId);
    RECT r={12,8,right,32};pDrawTextW(dc,headingLine,-1,&r,DT_LEFT|DT_TOP|DT_SINGLELINE|DT_NOPREFIX);
    WCHAR countLine[160];CopyW(countLine,160,L"Verfügbar: ");AppendSignedW(countLine,160,snap.totalAvailable);if(snap.sourceMissions&&snap.scannedMissions<snap.sourceMissions){AppendCharW(countLine,160,L' ');AppendCharW(countLine,160,L'(');AppendDecW(countLine,160,snap.scannedMissions);AppendCharW(countLine,160,L'/');AppendDecW(countLine,160,snap.sourceMissions);AppendCharW(countLine,160,L')');}
    pSelectObject(dc,g_smallFont);pSetTextColor(dc,RGBc(184,202,219));r.left=12;r.top=34;r.right=right;r.bottom=54;pDrawTextW(dc,countLine,-1,&r,DT_LEFT|DT_TOP|DT_SINGLELINE|DT_NOPREFIX);
    int y=58;int left=12;int gap=10;int width=(right-left-gap)/2;if(width<120)width=120;
    if(snap.displayedRows==0&&snap.status[0]){pSelectObject(dc,g_smallFont);pSetTextColor(dc,RGBc(184,202,219));RECT sr={left,y,right,y+48};pDrawTextW(dc,snap.status,-1,&sr,DT_LEFT|DT_TOP|DT_WORDBREAK|DT_NOPREFIX);y+=54;}
    else{
        ...
        for(u32 i=0;i<shown;++i){u32 col=i/rows;u32 line=i%rows;int x=left+(int)col*(width+gap);int yy=y+(int)line*24;RECT tr={x,yy,x+width,yy+22};pDrawTextW(dc,snap.rows[i].title,-1,&tr,DT_LEFT|DT_TOP|DT_SINGLELINE|DT_NOPREFIX|DT_END_ELLIPSIS);}
        y+=((int)rows)*24;
        if(snap.totalAvailable>(s32)shown){pSetTextColor(dc,RGBc(184,202,219));pSelectObject(dc,g_smallFont);RECT mr={left,y+4,right,y+30};pDrawTextW(dc,L"Weitere Aufträge sind vorhanden.",-1,&mr,DT_LEFT|DT_TOP|DT_SINGLELINE|DT_NOPREFIX);}
    }
