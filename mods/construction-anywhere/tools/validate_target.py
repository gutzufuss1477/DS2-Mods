...
PATCHES = (
    Patch(
        ...
        0x01337820,
        bytes.fromhex(
            "40 53 41 54 41 57 48 81 EC 10 04 00 00 48 8B 05 CC 1F 68 02"
        ),
    ),
    Patch(
        "PermittedArea",
        0x01336530,
        bytes.fromhex(
            "48 89 5C 24 08 57 48 83 EC 40 48 8B 5A 08 48 8B FA E8 6A A5"
        ),
    ),
    Patch(
        "UnconstructableArea",
        0x01339AE0,
        bytes.fromhex(
            "4C 8B DC 49 89 5B 20 56 57 41 54 41 56 41 57 48 81 EC 30 03"
        ),
    ),
)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the DS2 Construction Anywhere target")
    ...
    try:
        for patch in PATCHES:
            actual = pe.read_rva(patch.rva, len(patch.signature))
            count = pe.data.count(patch.signature)
            signature_ok = actual == patch.signature
            unique_ok = count == 1
            checks[f"{patch.name}_signature"] = signature_ok
            checks[f"{patch.name}_unique"] = unique_ok
            print(f"{patch.name} @ 0x{patch.rva:X}: {actual.hex(' ').upper()}")
            print(f"{patch.name} occurrences: {count}")
