...
$source = Join-Path $root 'src\chiral_bandwidth_costs.cpp'
$definitions = Join-Path $root 'src\kernel32.def'
$configuration = Join-Path $root 'config\ds2_chiral_bandwidth_costs.ini'
...
$object = Join-Path $out 'chiral_bandwidth_costs.obj'
...
$asi = Join-Path $out 'ds2_chiral_bandwidth_costs.asi'
...
Copy-Item -LiteralPath $configuration -Destination (Join-Path $out 'ds2_chiral_bandwidth_costs.ini') -Force
